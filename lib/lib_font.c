 ...
 * lib_font.c
 *
 *  库层实现：4 套 ASCII 点阵字模（列行式），纯数据、无硬件依赖。
 *
 *  特性：
 *    - 字模数据移植自波特律动 OLED 驱动的字库（MIT License），逐字节未改动
 *      （仅 8x6 的三个缺失字模参照其 12x6 字形补齐，见下）。
 *    - 每套字库从空格 ' ' 起连续排列共 95 个可打印字符（' '~'~'），
 *      取模按 (ch - ' ') 索引即可。
 *    - 列行式排布：先列后行，每列按字节自上而下；与 dev_oled_set_block 对齐。
 *
 *  分层原则：
 *    - 本文件只有字模数组与全局字体实例，不含任何逻辑与平台依赖。
 *    - 换取模工具/追加中文字库只动本文件，上层（dev_oled）无需修改。
 *
 *  与参考字库的差异：
 *    - 参考的 8x6 字库缺 '|'/'}'/'~' 三个字模（打印会越界读到下一个数组），
 *      已参照其 12x6 字形补齐；其余字模逐字节未改动。
 *    - 中文字库可自行用取模工具生成后追加（见 lib_font.h 说明）。
 *
 *  注意：
 *    - 字模数据为规整的十六进制表，禁用格式化工具重排（clang-format off），
 *      否则 diff 会一片红、字模对应关系也会看不出来。
 */

#include "lib/lib_font.h"

/* 字模数据为规整的十六进制表，禁用格式化工具重排 */
/* clang-format off */

/* ============ 1. 8x6 ASCII 字模（宽6 x 高8，列行式） ============ */

static const unsigned char ascii_8x6[][6] = {
    ...
    {0x14, 0x14, 0x14, 0x14, 0x14, 0x14}, // { 位置：参考工程原为"水平线"自定义字形，按原样保留
    {0x00, 0x00, 0x00, 0xFF, 0x00, 0x00}, // |    （参考工程缺此 3 个字模，打印会越界读下一数组，
    {0x00, 0x41, 0x77, 0x08, 0x00, 0x00}, // }     此处参照 12x6 字形补齐，共 95 个可打印字符）
    {0x08, 0x04, 0x08, 0x10, 0x10, 0x08}, // ~
};

/* ============ 2. 12x6 ASCII 字模（宽6 x 高12，列行式） ============ */

static const unsigned char ascii_12x6[][12] = {
...
/* ============ 3. 16x8 ASCII 字模（宽8 x 高16，列行式） ============ */

static const unsigned char ascii_16x8[][16] = {
...
/* ============ 4. 24x12 ASCII 字模（宽12 x 高24，列行式） ============ */

static const unsigned char ascii_24x12[][36] = {
...
/* 全局字体实例（字模从空格 ' ' 起连续排列，共 95 个可打印字符） */
const font_ascii_t g_font8x6   = {8u,  6u,  (const uint8_t *)ascii_8x6};
const font_ascii_t g_font12x6  = {12u, 6u,  (const uint8_t *)ascii_12x6};
const font_ascii_t g_font16x8  = {16u, 8u,  (const uint8_t *)ascii_16x8};
const font_ascii_t g_font24x12 = {24u, 12u, (const uint8_t *)ascii_24x12};